...
//Model for every venue retrieved from foursquare
VenueModel = function(data) {
  ...
  //each venue has a marker associated with it
...
//Check that there is a valid value for each property of the venue model
...
// Array of venue models
function VenuesModel() {
  var self = this;

  //Flag if ajax call fails or not
  self.ajax_failed = ko.observable(false);
  //Set for number of models
  self.num_unread = ko.observable(0);
  self.num_total = ko.observable(0);
  //categories object
  self.categories = {};
  //filter keyword
  self.category_filter = ko.observable("None");
  //array for all VenueModel
  ...
  self.addvenuesModel = function() {
    ...
    $.getJSON(urlToRequest, function(data) {
      ...
      if(self.checkError(venues) === false) {
        alert(
          "No results found.\nPlease try a different point of interest category."
          );
        return;
      }
    ...
    //don't show the list if the ajax call fails
    }).error(function() {
...
function initialize() {
  ...
  $("#setting").popover({
    ...
    content: "<input id='loc_input' type='text' placeholder='Search a different location'>",
    html: true,
    placement: "bottom"
  });
  //listButton collapse
  ...
  if (map.getCenter() === undefined) {
    map.setCenter(new google.maps.LatLng(59.6400705,-151.601765));
  }
  ...
  $("#setting").on('shown.bs.popover', function() {
    ...
    google.maps.event.addListener(autocomplete, 'place_changed', function() {
      ...
      //No geometry
      if(!place.geometry) {
        alert("Unable to locate, Ensure you are selecting the full name.");
        return;
      }
      //Have geometry
      //If have viewport
...
//initialize function every time the window loads
